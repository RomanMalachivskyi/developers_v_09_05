	...
	protected void doPost(HttpServletRequest request,
		...
		if (user == null) {
			...
			if (!user.getPassword().contentEquals(Register.MD5Generate(password))) {
				...
				for(User user2: DataHolder.getInstance().getUserSessions().values()){//перевіряє чи такий користувач вже залогінений
				...
				if(isUserExists){
					...
					response.getWriter().println("such login already logged");
						...
						if(null!=playingUser){
							...
							DataHolder.getInstance().getUserSessions().put(session, user);//створення нової сесії
						}
				}
			}
		}
