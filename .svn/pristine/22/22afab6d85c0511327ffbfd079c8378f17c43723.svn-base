...
public class PassiveObjectDAO extends MapItemDAOJdbc<MapObject> {
	...
	private String getObjectsByMapId = "SELECT passive_object.rotation_angle, passive_object.i, passive_object.j, object.path, object.name, object.id "
			+ "FROM passive_object INNER JOIN object ON passive_object.id_map = ? "
	...
	private List<MapObject> createObjectList(ResultSet rs) throws SQLException{
		...
		while(rs.next()){
			...
			switch(rs.getInt("id")){
			case 2 :
				objs.add(new WallCorner(rs.getInt("i"), rs.getInt("j"), rs.getInt("rotation_angle"), rs.getString("path")));
				break; 
			case 1:
